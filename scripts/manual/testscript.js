...
d.bullets([
  ...
  "Higher mathematics gets a SECOND opinion: an independent computer-algebra engine (mathjs, sharing no code with the question generators) re-derives grade 8\u201312 answers from exactly the text a student sees \u2014 solving equations by substitution, comparing algebraic forms at sample points, taking limits and derivatives numerically, integrating, and checking trig values. About 4,500 of the 8,000 senior questions per sweep are verifiable this way; the rest are conceptual or vocabulary questions with nothing to compute. This auditor caught a real bug on its first day (a derivative displayed 23x\u00B2 where the answer was 6x\u00B2), and the test suite feeds it planted wrong answers that it MUST catch \u2014 so it can never quietly become a rubber stamp.",
]);
...
d.h2("Running the CAS audit yourself");
d.code("cd C:\\Users\\buruf\\Documents\\Pedmas\nnpm run audit:cas          # one-line verdict: checked / passed / failed, plus per-checker counts\nnpm run audit:cas:report   # per-family table: grade span, questions, checked, passed, coverage");
d.p("Expected result: FAILED 0. Any failure prints the exact skill, stage, prompt and answer — that is either a real content bug (fix the generator) or a question whose wording changed (update the checker). Run it after any change to the question generators for grades 8\u201312.");

d.h1("I. Automated checks (run before every deploy)");
d.code("cd C:\\Users\\buruf\\Documents\\Pedmas\nnpm test                 # 284 tests: every skill x stage x region, every lesson step, CAS sweep\nnpm run audit:cas        # full CAS second opinion on grades 8-12 (expect FAILED 0)\nnpm run build            # must compile clean — stop the dev server first");
