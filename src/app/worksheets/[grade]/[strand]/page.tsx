...
export default async function WorksheetPage({
  ...
  // Two showcase sheets are fully public; every other page is a short
  // preview without its answer key — the full library is worth an account
  // (the owner's call: an infinite generator is too much to give away).
  // Rerolls (?seed=) are account-only everywhere. All of it enforced here,
  // server-side: the locked questions are never sent to the browser.
  const canReroll = Boolean(account);
  const unlocked = canReroll || isFreeWorksheet(grade, slug);
  const seed =
  ...
  const full = buildWorksheet(grade, slug, seed, region);
  if (!full) notFound();
  const sheet = unlocked
    ? full
    : { ...full, questions: full.questions.slice(0, WORKSHEET_PREVIEW_SIZE) };
    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 print:max-w-none print:p-0">
        <div className="print:hidden">
          ...
          <div className="mt-5">
            <WorksheetToolbar canReroll={canReroll} unlocked={unlocked} signupsOpen={signupsOpen} />
          </div>
        </div>
          ...
          {!unlocked && (
            <div className="mt-4 rounded-xl border border-brand-200 bg-brand-50 px-5 py-4 text-center">
              <p className="font-bold text-ink-900">
                That was {WORKSHEET_PREVIEW_SIZE} of {WORKSHEET_SIZE} questions.
              </p>
              <p className="mt-1 text-sm text-ink-700">
                The full sheet, the answer key, and unlimited fresh sheets for every grade and
                topic are free with a PEDMAS account.
              </p>
              <Link
                href={signupsOpen ? "/signup" : "/login"}
                className="btn mt-3 inline-flex items-center rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700"
              >
                {signupsOpen ? "Create a free account" : "Log in to unlock"}
              </Link>
            </div>
          )}
